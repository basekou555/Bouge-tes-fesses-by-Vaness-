...
- Arbitrages : chaque saison commence par `pendingChoice='priorities'` (`FOCUS_AREAS` / `PFOCUS_AREAS`, deux choix sur cinq) ; `focusScaled()` applique des rendements décroissants aux gains et amortit les pertes près du bas, pour que rien ne tende vers 0 ni vers 100. La jauge `proches` (coach) et `entourage` (joueur·euse) amortissent ou amplifient les montées de pression dans `coachApplyEffects`. Chaque option d'événement affiche son coût via `effectChips()` dans `ui.js` ; un choix peut porter une `seed` que `plantSeed()` range dans `state.seeds` et que `coachRipeSeed()` fait germer deux ou trois saisons plus tard.
